...
def _to_out(c: Campagne, db: Session) -> CampagneOut:
    ...
    wilayas_ids = [w.wilaya_id for w in c.wilayas] if hasattr(c, 'wilayas') else []
...
def _sync_wilayas(db: Session, campagne_id: int, wilaya_ids: List[int]):
    ...
    db.query(CampagneWilaya).filter(CampagneWilaya.campagne_id == campagne_id).delete()
    for wid in wilaya_ids:
        db.add(CampagneWilaya(campagne_id=campagne_id, wilaya_id=wid))
