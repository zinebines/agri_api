// ===== INITIALIZATION =====
        ...
        function toggleLanguage() {
            ...
            // Update all text nodes (handle elements that contain child HTML)
            document.querySelectorAll('.lang-text').forEach(el => {
                const text = el.getAttribute('data-' + currentLang);
                if (!text) return;
                if (el.children.length > 0 && !el.classList.contains('chat-msg')) {
                    el.innerHTML = text;
                } else {
                    el.textContent = text;
                }
            ...
            // Update standalone toggle button label
            const label = document.getElementById('lang-toggle-label');
            if (label) label.textContent = isAr ? 'FR / Français' : 'AR / عربي';

            // Update page title
            document.title = isAr
                ? 'وزارة الفلاحة - الإحصاء الفلاحي | رؤية 2026'
                : "Ministère de l'Agriculture | Vision 2026";
        }
...
// toggleLanguage, toggleChat, sendChat are defined once above — no duplicates

// (toggleChat defined once above)

// (sendChat defined once above)
